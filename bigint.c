...
void Karatsuba(bigint** result, bigint* A, bigint* B)
{
    ...
    // Aj, Bj는 len개의 워드열
    BI_New(&A0, len); // A0를 담을 bigint 생성
    BI_New(&A1, len); // A1를 담을 bigint 생성
    BI_New(&B0, len); // B0를 담을 bigint 생성
    BI_New(&B1, len); // B1를 담을 bigint 생성
    ...
    BI_New(&temp1, A_Len); // A 담을 temp1
    BI_New(&temp2, A_Len); // A 담을 temp2
    BI_New(&temp3, B_Len); // B 담을 temp3
    BI_New(&temp4, B_Len); // B 담을 temp4
    ...
    Right_Shift(temp3, len);
    Reduction_BI(&temp4, len);
    ...
    for (i = 0; i < len; i++)
    {
        A1->a[i] = temp1->a[i]; // A1 = A >> (len * w)
        A0->a[i] = temp2->a[i]; // A0 = A mod 2^(len * w)

        B1->a[i] = temp3->a[i]; // B1 = B >> (len * w)
        B0->a[i] = temp4->a[i]; // B0 = B mod 2^(len * w)
    }
