...
int squaring_karatsuba_test()
{
	...
	for (i = 0; i < 1; i++)
	{
		...
		SQUC_Karatsuba(&bi_re, bi_1); /********************** 1. 여기 타고 들어가면 **************************/
		BI_Refine(bi_re);
...
void BI_New(bigint** x, int wordlen)
{
	...
	*x = (bigint*)malloc(sizeof(bigint));
	...
	(*x)->a = (word*)calloc(wordlen, sizeof(word)); // bigint x->a 생성
}
...
void ADDC(bigint** C, bigint** A, bigint** B, int sign)
{
	...
	(*B)->wordlen = A_Len;
	temp = (word*)realloc((*B)->a, sizeof(word) * (*B)->wordlen); /********************** 4. B의 워드열의 길이를 늘려 줄 때 중단 **************************/
	if (temp != NULL)
		(*B)->a = temp;
...
/*
	...
	9 :
*/
void SQUC_Karatsuba(bigint** C, bigint* A) /********************** 2. 디버깅 상태에서 계속 F10 누르고 **************************/
{
	int i;
	int flag = 1;
	int len, A_Len;
	...
	ADDC(C, &R, &S, R->sign); // C = R + S /********************** 3. 이 함수 안으로(F11) 들어가면 **************************/
